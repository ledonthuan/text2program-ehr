...
class DataCollatorForLanguageModeling:
    ...
    def __call__(
        ...
        if isinstance(examples[0], dict):
            ...
            # Add answers as a list (for validation/metrics)
            if 'answers' in examples[0]:
                batch['answers_cpu'] = [examples[idx]['answers'] for idx in range(len(examples))] #batch['answers'] = [examples[idx]['answers'] for idx in range(len(examples))]
        ...
        # if 'answers' in batch:
        #     print("Removing 'answers' from batch before return")
        #     del batch['answers']
        return batch
...
class DataCollatorForUniLMGeneration:
    ...
    def __call__(
        ...
        if isinstance(examples[0], dict):
            ...
            SKIP_KEYS = ['answers']  # Add more keys if needed
            batch = {}
            for k in examples[0].keys():
                if k in SKIP_KEYS:
                    continue
        ...
        if self.mlm:
            if self.attention_mask_type == 's2s':
                ...
                # trace_indices = batch["token_type_ids"]
                batch["text_labels"] = torch.full_like(torch.tensor(batch["labels"]), -100)
...
class DataCollatorForBertGeneration:
    ...
    def __call__(self, examples: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        ...
        # print("Decoder input ids for this batch:")
        # for i, e in enumerate(examples):
        #     print(f"Example {i}: {e['decoder_input_ids']}")
        ...
        batch = dict(batch)
        if 'answers' in examples[0]:
            batch['answers_cpu'] = [e['answers'] for e in examples]

            # Do NOT add batch['answers'] = ...
        ...
        #             filtered_answers.append(float(answer))
        #         except ValueError:
        #             continue
        #     elif isinstance(answer, int):  # optionally include ints if desired
        #         filtered_answers.append(float(answer))

        # if filtered_answers:
        #     batch['answers'] = torch.tensor(filtered_answers)
            
        # print("Batch keys:", batch.keys())
        # if 'answers' in batch:
        #     print("WARNING: batch['answers'] is present and type is", type(batch['answers']))
